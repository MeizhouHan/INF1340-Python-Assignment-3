...
def replace_missing_with_nan(data: np.array, column_name: str, missing_codes: Tuple[float]) -> None:
    ...
    nan
    '''
    new_array = data[column_name]
    for item in missing_codes:
        new_array[new_array == item] = np.nan        
...
def write_categorical_csv(data: np.array, column_name: str, outfile: TextIO) -> None:
    ...
    M,668
    '''
    label, counts = np.unique(data[column_name], return_counts=True)
    outfile.write(column_name + ',count\n')
    for i in np.arange(len(label)):
        outfile.write(str(label[i]) + ',' + str(counts[i]) + '\n')    
...
def write_column_summary_csv(data: np.array, column_name: str, outfile: TextIO) -> None:
    ...
    '''
    array = data[column_name]
    median = str(np.nanmedian(array))
    if column_name in INTERVAL + RATIO:
        mean, std = str(np.nanmean(array)), str(np.nanstd(array))
        outfile.write(column_name+','+median+','+mean+','+std+'\n')       
